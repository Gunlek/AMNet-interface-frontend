...
const BlackUl = dynamic<any>(() => import("../Text/style").then((mod) => mod.BlackUl));
  ...
  return (
    ...
    >
      ...
      {process.env.NEXT_PUBLIC_FAQ_STATE === 'inactive' ?
        ...
        <>
          Rdv sur la page
          <StyledLink href="/homepage/faq" target="_blank" color={props.color} rel="noreferrer">{" "}FAQ</StyledLink>
          {" "}ou par
        </>
      }
      mail à{" "}
    ...
    >
      ...
      <AnimatePresence>
        {display &&
          ...
          >
            Petite thuysse pour éditer l&apos;adresse MAC d&apos;une personne :
            <BlackUl>
              <li>PC : double-clic dessus</li>
              <li>Téléphone : un appui long</li>
            </BlackUl>
          </StyledAdminToolTip>
        }
      </AnimatePresence>
      ...
      <AnimatePresence>
        {display &&
          <StyledGuestToolTip
            ...
            transition={{ ease: "linear", duration: 0.3 }}
          >
            Les identifiants du compte invité pour AMNet Wi-Fi :
            <BlackUl>
              <li>Identifiant : {process.env.NEXT_PUBLIC_GUEST_LOGIN}</li>
              <li>Mot de passe : {process.env.NEXT_PUBLIC_GUEST_PASSWORD}</li>
            </BlackUl>
          </StyledGuestToolTip>
        }
      </AnimatePresence>
